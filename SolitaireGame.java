...
import com.example.demoxxx.Movimiento;
import com.example.demoxxx.Pila;

import java.util.ArrayList;
/** * Juego de solitario. * * @author (Cecilia Curlango Rosas) * @version (2025-2) */
...
public class SolitaireGame {
    ...
    DrawPile drawPile;
    WastePile wastePile;
    Pila<Movimiento> historial = new Pila<>(200);
    ...
    public void undo() {
        ...
        switch (mov.getTipo()) {
            ...
            case "WASTE_TABLEAU":
                TableauDeck destino = (TableauDeck) mov.getDestino();
                WastePile origen = (WastePile) mov.getOrigen();
                CartaInglesa carta = destino.removerUltimaCarta();

                ArrayList<CartaInglesa> lista = new ArrayList<>();
                lista.add(carta);

                origen.addCartas(lista);

                break;
            case "WASTE_FOUNDATION":
                FoundationDeck foundation = (FoundationDeck) mov.getDestino();
                WastePile waste = (WastePile) mov.getOrigen();

                CartaInglesa cartaF = foundation.removerUltimaCarta();

                ArrayList<CartaInglesa> lista2 = new ArrayList<>();
                lista2.add(cartaF);

                waste.addCartas(lista2);
                break;
            case "TABLEAU_FOUNDATION":
                FoundationDeck f = (FoundationDeck) mov.getDestino();
                TableauDeck t = (TableauDeck) mov.getOrigen();
                CartaInglesa cartaTF = f.removerUltimaCarta();

                if (cartaTF != null) {
                    if (!t.isEmpty()) {
                        CartaInglesa cartaDebajo = t.getUltimaCarta();
                        cartaDebajo.makeFaceDown();
                    }
                    t.getCards().add(cartaTF);
                    cartaTF.makeFaceUp();
                }
                break;
            case "TABLEAU_TABLEAU":
                TableauDeck dest = (TableauDeck) mov.getDestino();
                TableauDeck orig = (TableauDeck) mov.getOrigen();
                ArrayList<CartaInglesa> cartas = mov.getCartas();
                if (!orig.isEmpty()) {
                    orig.getUltimaCarta().makeFaceDown();
                }
                for (int i = 0; i < cartas.size(); i++) {
                    dest.removerUltimaCarta();
                }
                orig.getCards().addAll(cartas);
                break;
        }
    }
